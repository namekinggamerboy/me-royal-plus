...
client.on('guildMemberAdd', async member => {
    ...
    const link = member.user.displayAvatarURL({ format: 'png', size: 2048 });
const result = await ftch(link);
const avatar = await result.buffer();
 const lin = config[member.guild.id].welcomeimage;
const aa = await ftch(lin);
 const an = await aa.buffer();     
      
  let chann = member.guild.channels.find(e => e.name === config[member.guild.id].welcomechannel);
    chann.send("",
       new Discord.MessageAttachment(
    new Canvas(1024, 450)
.addImage(an, 0, 0, 1024, 450)
.save()
.setColor(config[member.guild.id].welcomecolor)
.addCircle(530, 150, 130)
.addRoundImage(avatar, 410, 30, 240, 240, 120)
.save()
.setColor(config[member.guild.id].welcometext)
.setTextFont("bold 75px robot")
.addText("WELCOME", 325, 350)
.save()
.addRoundImage(avatar, 410, 30, 240, 240, 120)
.save()
.setColor(config[member.guild.id].welcomecolor)
.setTextFont("bold 39px sans-serif")
.addText(member.user.username, 370, 395)
        .toBuffer(),
      "welcome.png"    ) 
    );    
    }
